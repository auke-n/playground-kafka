...
- The demo UI renders a stage as `Queued for Kafka` immediately after the API accepts it, then replaces that optimistic state with the broker-confirmed record. The observer consumer independently verifies the record without duplicate timeline entries.
